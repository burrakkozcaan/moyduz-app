...
export default async function MarketplaceTemplatesPage({
  ...
  return (
    <main className="min-h-screen flex-1 bg-ln-gray-250 text-white">
      <script
      ...
      <div className="container mx-auto max-w-7xl px-4 py-12 md:px-6 md:py-16">
        {/* Header */}
        <div className="mb-12">
          <h1 className="mb-4 text-[34px] font-550 leading-[40px] -tracking-[0.02em] text-ln-gray-900 dark:text-white md:text-5xl lg:text-6xl">
            Şablonlar
          </h1>
          <p className="max-w-2xl text-ln-paragraph-md text-ln-gray-700 dark:text-white/70 lg:text-ln-paragraph-lg">
        ...
        {categories.length > 0 && (
          <div className="mb-16">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-550 text-ln-gray-900 dark:text-white">Kategoriler</h2>
              <Link
                href="/marketplace/templates/category"
                className="text-ln-label-sm text-ln-gray-700 dark:text-white/70 transition hover:text-ln-gray-900 dark:hover:text-white"
              >
                Tümünü Gör →
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4 xl:grid-cols-5">
              {categories.map((cat) => (
                <Link
                  ...
                  className="group flex flex-col overflow-hidden rounded-lg bg-ln-gray-100 dark:bg-[#1C1C1C] transition hover:bg-ln-gray-200 dark:hover:bg-[#262626]"
                >
                      ...
                      return (
                        <>
                          ...
                          {img2 ? (
                            ...
                            <div className="hidden aspect-[4/3] rounded-[10px] bg-white/10 md:block" />
                          )}
                        </>
                      )
                    })()}
                  ...
                  <div className="p-[15px]">
                    ...
                    <p className="mt-0 text-ln-paragraph-xs text-ln-gray-700 dark:text-white/70">
                      {(() => {
            ...
            <div className="flex flex-wrap gap-3">
                <Link
                  ...
                  selectedCategory === null && selectedPrice === null
                    ? 'border-white/10 bg-white text-ln-gray-900 dark:text-white'
                    : 'border-white/10 bg-ln-gray-925 dark:bg-[#1C1C1C] text-white hover:bg-ln-gray-950 dark:hover:bg-[#262626]'
                }`}
              >
                Tümü
              </Link>
              ...
              >
                ...
                <div className="p-3">
                  <div className="mb-2 flex items-start justify-between">
                    <h3 className="line-clamp-1 text-ln-label-md font-550 text-ln-gray-900 dark:text-white group-hover:text-white/90">
                      {t.title}
                    </h3>
                  </div>
                  {t.designer && (
                    <p className="text-ln-paragraph-xs text-ln-gray-700 dark:text-white/70">
                  ...
                  {t.tags && t.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {t.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="rounded bg-white/10 px-2 py-0.5 text-[10px] text-ln-gray-700 dark:text-white/70"
                        >
                          {tag}
                        </span>
                      ))}
