...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!page) {
    return { title: 'Multi-Vendor Marketplace | Moyduz', robots: { index: false } }
  }

  const { frontmatter } = page
  const title = frontmatter.title || 'Multi-Vendor Marketplace | Moyduz'
  const description =
    (frontmatter.meta_description as string) ||
    'Kendi pazaryerinizi kurun. Çok satıcılı marketplace platformu.'
  const url =
    (frontmatter.canonical_url as string) || 'https://moyduz.com/multi-vendor'
...
export default async function MultiVendorPage() {
  const page = await getPage('multi-vendor')
