...
const ITEMS = [
  {
    ...
    label: 'Web Tasarım',
    description:
      'Markanıza özel, dönüşüm odaklı arayüzler tasarlıyoruz. Her piksel kullanıcı deneyimini ve satış performansını artırmak üzere kurgulanır.',
  },
  {
    id: 'router' as const,
    label: 'Web Geliştirme',
    description:
      'Next.js ve React ile hızlı, güvenilir ve ölçeklenebilir web uygulamaları geliştiriyoruz. Temiz kod, modern mimari, yüksek performans.',
  },
  {
    id: 'database' as const,
    label: 'E-Ticaret',
    description:
      'Ödeme, kargo ve sipariş yönetimini içeren tam donanımlı e-ticaret sistemleri kuruyoruz. Siz satışa odaklanın, teknik kısım bizde.',
  },
  {
    id: 'tab' as const,
    label: 'SaaS & B2B',
    description:
      'Rezervasyon, CRM ve kurumsal SaaS platformları geliştiriyoruz. Sektörünüze özel, büyüdükçe ölçeklenen yazılım mimarileri.',
  },
  {
    id: 'mobile' as const,
    label: 'SEO & Destek',
    description:
      'Core Web Vitals uyumlu altyapı, teknik SEO ve teslim sonrası öncelikli destek. Lansmandan sonra da yanınızdayız.',
  },
];

type ItemId = (typeof ITEMS)[number]['id'];

const ORIGINS: Record<ItemId, string> = {
  server: '205px 120px',
  router: '293px 184px',
  database: '393px 228px',
  tab: '200px 393px',
  mobile: '100px 328px',
};

const DEVICE_LABELS: Record<ItemId, { x: number; y: number; text: string }> = {
  server:   { x: 206, y: 57,  text: 'Tasarım' },
  router:   { x: 294, y: 134, text: 'Geliştirme' },
  database: { x: 393, y: 165, text: 'E-Ticaret' },
  tab:      { x: 100, y: 268, text: 'SaaS' },
  mobile:   { x: 200, y: 344, text: 'Destek' },
};

function svgLabel(x: number, y: number, text: string, active: boolean) {
...
export default function PlatformPillars() {
  ...
  function gStyle(id: ItemId): React.CSSProperties {
  ...
  return (
    <section className='py-24'>
      <div className='mx-auto max-w-5xl px-6'>
        {/* Header */}
        <div className='mb-12 grid items-end gap-4 md:grid-cols-2'>
          <h2 className='text-4xl font-semibold leading-tight text-ln-gray-900 dark:text-white'>
            Uçtan uca dijital çözümler
          </h2>
          <p className='text-lg text-ln-gray-500'>
            Web tasarımdan e-ticarete, SaaS&apos;tan SEO&apos;ya — tüm dijital ihtiyaçlarınızı
        ...
        <div className='grid items-center lg:grid-cols-5'>
          ...
          <div className='relative z-10 lg:col-span-2'>
            ...
            <div className='absolute inset-y-0 flex items-center justify-center gap-3 max-sm:-inset-x-4 max-sm:justify-between sm:flex-col lg:-left-8 lg:-translate-x-full'>
              <button
                ...
                className='inline-flex h-9 w-9 items-center justify-center rounded-full border border-transparent bg-white shadow-sm ring-1 ring-black/10 transition-colors hover:bg-ln-gray-50 disabled:pointer-events-none disabled:opacity-40 dark:bg-ln-gray-900 dark:text-white dark:ring-white/15'
              >
              ...
              {ITEMS.map((item, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ width: isActive ? '100%' : 'fit-content' }}
                    className={`relative min-w-0 max-w-xs cursor-pointer overflow-hidden rounded-3xl text-left ring transition-colors duration-500 max-md:mx-auto ${
                      isActive
                        ? 'bg-white shadow-md ring-ln-gray-200 dark:bg-ln-gray-800/50 dark:ring-ln-gray-700'
                        : 'text-ln-gray-500 ring-ln-gray-200 hover:text-ln-gray-900 dark:text-ln-gray-400 dark:ring-ln-gray-700 dark:hover:text-white'
                    }`}
                  >
                    {isActive ? (
                      <div className='px-6 py-[18px]'>
                        <p className='max-w-md text-ln-gray-500 dark:text-ln-gray-400'>
                          <strong className='font-medium text-ln-gray-900 dark:text-white'>
                            {item.label}.
                          </strong>{' '}
                          {item.description}
                        </p>
                      ...
                      <button className='flex h-10 cursor-pointer items-center gap-2 px-4'>
                        <CirclePlus className='size-3.5' />
                        <h3 className='text-nowrap text-sm font-medium'>{item.label}</h3>
                      </button>
                    )}
                  </div>
                );
          ...
          <div
            className='
              [--color-background:white] [--color-border:#e5e7eb] [--color-layer-border:rgba(0,0,0,0.15)]
              [--color-layer-border-2:rgba(0,0,0,0.5)] [--color-muted:#f3f4f6] [--color-primary:#f97316]
              dark:[--color-background:#111827] dark:[--color-border:#374151] dark:[--color-layer-border:rgba(255,255,255,0.15)]
              dark:[--color-layer-border-2:rgba(255,255,255,0.4)] dark:[--color-muted:#1f2937]
              max-lg:row-start-1 lg:col-span-3 lg:-translate-x-20
            '
          >
            <svg
              viewBox='0 0 627 441'
            ...
            >
              ...
              {/* Device: Server → Web Tasarım */}
              <g style={gStyle('server')}>
                <path
                ...
                <path
                  ...
                  fill='var(--color-primary)'
                />
                {svgLabel(DEVICE_LABELS.server.x, DEVICE_LABELS.server.y, DEVICE_LABELS.server.text, activeId === 'server')}
              </g>

              {/* Device: Router → Web Geliştirme */}
              <g style={gStyle('router')}>
                <path
                ...
                <path
                  ...
                  fill='var(--color-layer-border)'
                />
                {svgLabel(DEVICE_LABELS.router.x, DEVICE_LABELS.router.y, DEVICE_LABELS.router.text, activeId === 'router')}
              </g>

              {/* Device: Database → E-Ticaret */}
              <g style={gStyle('database')}>
                <path
                ...
                <path
                  ...
                  fill='var(--color-primary)'
                />
                {svgLabel(DEVICE_LABELS.database.x, DEVICE_LABELS.database.y, DEVICE_LABELS.database.text, activeId === 'database')}
              </g>

              {/* Device: Mobile → SEO & Destek */}
              <g style={gStyle('mobile')}>
                <path
                ...
                <circle
                  ...
                  stroke='var(--color-layer-border)'
                />
                {svgLabel(DEVICE_LABELS.mobile.x, DEVICE_LABELS.mobile.y, DEVICE_LABELS.mobile.text, activeId === 'mobile')}
              </g>
              ...
              <g style={gStyle('tab')}>
                ...
                <path
                  ...
                  stroke='var(--color-layer-border)'
                />
                {svgLabel(DEVICE_LABELS.tab.x, DEVICE_LABELS.tab.y, DEVICE_LABELS.tab.text, activeId === 'tab')}
              </g>

              <defs>
                <filter id='labelShadow' x='-25%' y='-60%' width='150%' height='220%'>
                  <feDropShadow dx='0' dy='1' stdDeviation='2' floodOpacity='0.14' />
                </filter>
                <pattern
                ...
                >
                  <rect width='1' height='1' fill='var(--color-border)' />
                  <rect x='3' width='1' height='5' fill='var(--color-border)' />
                </pattern>
              </defs>
            </svg>
          </div>
