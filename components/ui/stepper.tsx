...
function Stepper({
  ...
  const focusNext = (currentIdx: number) =>
    focusTrigger((currentIdx + 1) % Math.max(1, triggerNodes.length));
  const focusPrev = (currentIdx: number) =>
    focusTrigger((currentIdx - 1 + triggerNodes.length) % Math.max(1, triggerNodes.length));
  ...
  const contextValue = React.useMemo<StepperContextValue>(
    ...
    [currentStep, handleSetActiveStep, stepsCount, orientation, registerTrigger, triggerNodes, indicators],
  );
...
function StepperIndicator({
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "relative flex items-center justify-center overflow-hidden size-6 shrink-0 rounded-full border border-ln-gray-200 dark:border-ln-gray-700 bg-ln-gray-100 dark:bg-ln-gray-800 text-ln-gray-600 dark:text-ln-gray-400 text-xs data-[state=completed]:bg-primary data-[state=completed]:text-primary-foreground data-[state=completed]:border-primary data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:border-primary",
        className,
      )}
    >
...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "m-0.5 rounded-full bg-ln-gray-200 dark:bg-ln-gray-700 group-data-[orientation=vertical]/stepper-nav:h-8 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1",
        className,
      )}
    />
  );
...
function StepperTitle({
  ...
  return (
    <h3
      ...
      data-state={state}
      className={cn("text-sm font-medium leading-none text-ln-gray-900 dark:text-ln-gray-0", className)}
    >
      {children}
    </h3>
  );
...
function StepperDescription({
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn("text-sm text-ln-gray-600 dark:text-ln-gray-400", className)}
    >
      {children}
    </div>
  );
...
function StepperNav({
  ...
  return (
    <nav
      ...
      data-orientation={orientation}
      className={cn(
        "group/stepper-nav inline-flex data-[orientation=horizontal]:w-full data-[orientation=horizontal]:flex-row data-[orientation=vertical]:flex-col data-[orientation=vertical]:gap-0",
        className,
      )}
    >
      {children}
...
export type { StepperProps, StepperItemProps, StepperTriggerProps, StepperContentProps };
