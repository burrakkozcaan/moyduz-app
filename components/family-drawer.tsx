...
function FamilyDrawerTrigger({
  ...
  return (
    <Drawer.Trigger asChild>
      <button
        className={clsx(
          'bg-background text-foreground hover:bg-accent focus-visible:shadow-focus-ring-button fixed left-1/2 top-1/2 h-[44px] -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full border px-4 py-2 font-medium antialiased transition-colors md:font-medium',
          className,
        )}
        type='button'
      >
...
function FamilyDrawerClose({
  ...
  const defaultClose = (
    <button
      data-vaul-no-drag=''
      className={clsx(
        'bg-muted text-muted-foreground focus-visible:shadow-focus-ring-button absolute right-8 top-7 z-10 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full transition-transform focus:scale-95 active:scale-75',
        className,
      )}
      type='button'
    >
...
function FamilyDrawerHeader({
  ...
  return (
    <header className={clsx('mt-[21px]', className)}>
      ...
      <p className='text-muted-foreground mt-3 text-[17px] font-medium leading-[24px] md:font-normal'>
        {description}
      </p>
    </header>
  );
