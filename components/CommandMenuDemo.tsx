...
import * as CommandMenu from '@/components/new-ui/command-menu';
import * as CompactButton from '@/components/new-ui/compact-button';
import * as Kbd from '@/components/new-ui/kbd';
import * as Tag from '@/components/new-ui/tag';
import { cn } from '@/utils/cn';

export interface CommandMenuDemoProps {
  open?: boolean;
  setOpen?: (open: boolean) => void;
  className?: string;
}

export function CommandMenuDemo({ open: controlledOpen, setOpen: controlledSetOpen }: CommandMenuDemoProps) {
  ...
  return (
    <>
      ...
      <CommandMenu.Dialog open={open} onOpenChange={setOpen}>
        <div className='group/cmd-input bg-white dark:bg-ln-gray-800 flex h-12 w-full items-center gap-2 px-5'>
          <RiSearch2Line
            className={cn(
              'text-text-soft-400 size-5 shrink-0',
              'transition duration-200 ease-out',
              'group-focus-within/cmd-input:text-ln-orange',
            )}
          />
          <CommandMenu.Input placeholder='Sayfa, şablon veya konu ara' />
          <Kbd.Root>
          ...
          <CompactButton.Root size='medium' variant='ghost' onClick={() => setOpen(false)}>
            <CompactButton.Icon as={RiCloseLine} />
          </CompactButton.Root>
        </div>

        <div className='px-5 py-4'>
          <div className='text-label-xs text-text-sub-600 mb-3'>Aranacak alanlar</div>
        ...
        <CommandMenu.List>
          <CommandMenu.Group heading='Hızlı işlemler'>
            <CommandMenu.Item value='fiyatlandirma' onSelect={() => go('/pricing')}>
              <CommandMenu.ItemIcon as={RiSparklingLine} className='text-ln-orange' />
              Fiyatlandırma sayfasını aç
            </CommandMenu.Item>
            <CommandMenu.Item value='sablonlar' onSelect={() => go('/marketplace/templates')}>
              <CommandMenu.ItemIcon as={RiSparklingLine} className='text-ln-orange' />
              Tüm şablonları incele
            </CommandMenu.Item>
            <CommandMenu.Item value='sss' onSelect={() => go('/faq')}>
              <CommandMenu.ItemIcon as={RiSparklingLine} className='text-ln-orange' />
              S.S.S. bölümüne git
            </CommandMenu.Item>
            <CommandMenu.Item value='iletisim' onSelect={() => go('/contact')}>
              <CommandMenu.ItemIcon as={RiSparklingLine} className='text-ln-orange' />
              İletişim sayfasını aç
            </CommandMenu.Item>
          </CommandMenu.Group>

          <CommandMenu.Group heading='Sonuçlar'>
            <CommandMenu.Item value='sonuc-sablonlar' onSelect={() => go('/marketplace/templates')}>
              <CommandMenu.ItemIcon as={RiFolderOpenLine} />
              Şablonlar
            </CommandMenu.Item>
            <CommandMenu.Item value='sonuc-fiyatlandirma' onSelect={() => go('/pricing')}>
              <CommandMenu.ItemIcon as={RiBankCardLine} />
              Fiyatlandırma
            </CommandMenu.Item>
            <CommandMenu.Item value='sonuc-faq' onSelect={() => go('/faq')}>
              <CommandMenu.ItemIcon as={RiBankLine} />
              S.S.S.
            </CommandMenu.Item>
            <CommandMenu.Item value='sonuc-blog' onSelect={() => go('/blog')}>
              <CommandMenu.ItemIcon as={RiHistoryLine} />
              Blog
            </CommandMenu.Item>
            <CommandMenu.Item value='sonuc-iletisim' onSelect={() => go('/contact')}>
              <CommandMenu.ItemIcon as={RiSearch2Line} />
              İletişim
            </CommandMenu.Item>
          </CommandMenu.Group>
        </CommandMenu.List>
        ...
        <CommandMenu.Footer>
          <div className='flex gap-3'>
            <div className='flex items-center gap-2'>
              ...
              <span className='text-paragraph-xs text-text-sub-600'>Gezin</span>
            </div>
            <div className='flex items-center gap-2'>
              <CommandMenu.FooterKeyBox>
                <RiCornerDownLeftLine className='size-4' />
              </CommandMenu.FooterKeyBox>
              <span className='text-paragraph-xs text-text-sub-600'>Seç</span>
            </div>
          </div>

          <div className='text-paragraph-xs text-text-sub-600 text-right'>
            Sonuç bulamazsan{' '}
            <button type='button' className='text-ln-orange underline' onClick={() => go('/contact')}>
              bize yaz
            </button>
          </div>
        </CommandMenu.Footer>
